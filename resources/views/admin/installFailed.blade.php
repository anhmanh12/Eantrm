...
@section('title', 'Install Failed')
@section('Title', 'Install Failed')
@section('installFailed')
<style>
div.container {
	width: 100%;
}
...
<?php
if ($packages == 0) {
	$data = DB::Connection('mysql2')->select('select * from customers where installed = ? ', array(2));
} else {
	$data = DB::Connection('mysql2')->select('select * from customers where installed = ? and package_id= ?', array(2, $packages));
}
