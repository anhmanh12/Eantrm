...
@section('title', 'Install Successfully')
@section('Title', 'Install Successfully')
@section('installSuccessfully')
<style>
div.container {
	width: 100%;
}
...
<?php
if ($packages == 0) {
	$data = DB::Connection('mysql2')->select('select * from customers where installed = ? and checked=?', array(1, 1));
} else {
	$data = DB::Connection('mysql2')->select('select * from customers where installed = ? and checked=? and package_id= ?', array(1, 1, $packages));
}
